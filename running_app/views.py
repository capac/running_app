...
class MainMenu(tk.Menu):
    ...
    def show_about(self):
        ...
        about_details = ('Running record application \
                         ...
                         concerning training progression \
                         and personal bests.\n\n\
...
class DataRecordForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks, *args, **kwargs):
        ...
        self.inputs['Location'] = w.LabelInput(runninginfo,
        ...
        self.searchbutton = w.LabelInput(command_section, 'Search',
                                         input_class=ttk.Button,
                                         input_var=self.
                                         callbacks['on_open_search_window'],
                                         input_args={'style': 'TButton'})
...
class DataInteractionForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks, api_data, *args, **kwargs):
        ...
        self.selectbutton = w.LabelInput(
            interactionpanel, 'Select', input_class=ttk.Button,
            input_args={'width': 10, 'style': 'TButton'},
        ...
        visibility = round(float(self.api_data['visibility'])/1e3, 1)
        self.visibility = ttk.Label(weatherpanel, text='Visibility: ' +
                                    str(visibility) + ' km',
                                    foreground='black',)
        self.visibility.grid(row=1, column=0, padx=2, pady=5, sticky=tk.W)
        self.temperature = ttk.Label(weatherpanel,
                                     text='Temperature: {0:>4} {1}C'.
                                     format(self.api_data['main_temp'],
                                            chr(176)),
                                     foreground='black',)
        self.temperature.grid(row=0, column=1, padx=2, pady=4, sticky=tk.W)
        feels_like = self.api_data['main_feels_like']
        self.temperature_feels_like = ttk.Label(
            weatherpanel, text='Feels like: {0:>10} {1}C'.format(feels_like,
                                                                 chr(176)),
            foreground='black',
            )
        self.temperature_feels_like.grid(row=1, column=1, padx=2,
                                         pady=5, sticky=tk.W)
        ...
        self.wind_speed = ttk.Label(weatherpanel,
                                    text='Wind speed: {0:>8} m/s'.
...
class SearchForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks,
        ...
        self.search_inputs['date_min'] = w.LabelInput(
            advancedselectioninfo, 'Date: min',
        ...
        self.search_inputs['date_min'].grid(row=0, column=0,
                                            padx=8, pady=(20, 0),
                                            sticky=(tk.W + tk.E))
        self.search_inputs['date_max'] = w.LabelInput(
            advancedselectioninfo, 'Date: max',
            field_spec=fields['Search date'],
            label_args={'foreground': 'black'},
            input_args={'width': 13,
                        'values': valid_dates,
        ...
        self.search_inputs['date_max'].grid(row=1, column=0, padx=8,
                                            sticky=(tk.W + tk.E))
        self.search_inputs['duration_min'] = w.LabelInput(
            advancedselectioninfo,
            'Duration: min',
            label_args={'foreground': 'black'},
            field_spec=fields['Search duration'],
            input_args={'width': 10},)
        self.search_inputs['duration_min'].grid(row=0, column=1,
                                                padx=8, pady=(20, 0),
                                                sticky=(tk.W + tk.E))
        self.search_inputs['duration_max'] = w.LabelInput(
            advancedselectioninfo,
            'Duration: max',
            label_args={'foreground': 'black'},
            field_spec=fields['Search duration'],
        ...
        self.search_inputs['distance_min'] = w.LabelInput(
            advancedselectioninfo, 'Distance: min',
            field_spec=fields['Distance'],
            label_args={'foreground': 'black'},
            input_args={'width': 10, 'max_var': max_dist_var,
                        'focus_update_var': min_dist_var, })
        self.search_inputs['distance_min'].grid(row=0, column=2,
                                                padx=8, pady=(20, 0),
                                                sticky=(tk.W + tk.E))
        self.search_inputs['distance_max'] = w.LabelInput(
            advancedselectioninfo, 'Distance: max',
            field_spec=fields['Distance'],
            label_args={'foreground': 'black'},
            input_args={'width': 10, 'min_var': min_dist_var,
                        'focus_update_var': max_dist_var, })
        self.search_inputs['distance_max'].grid(row=1, column=2, padx=8,
                                                sticky=(tk.W + tk.E))
        self.search_inputs['pace_min'] = w.LabelInput(
            advancedselectioninfo, 'Pace: min',
            field_spec=fields['Search pace'],
            label_args={'foreground': 'black'},
            input_args={'width': 10},)
        self.search_inputs['pace_min'].grid(row=0, column=3,
                                            padx=8, pady=(20, 0),
                                            sticky=(tk.W + tk.E))
        self.search_inputs['pace_max'] = w.LabelInput(
            advancedselectioninfo, 'Pace: max',
            label_args={'foreground': 'black'},
            field_spec=fields['Search pace'],
            input_args={'width': 10},)
        self.search_inputs['pace_max'].grid(row=1, column=3,
                                            padx=8, sticky=(tk.W + tk.E))
        min_speed_var = tk.DoubleVar(value='-infinity')
        max_speed_var = tk.DoubleVar(value='infinity')
        self.search_inputs['speed_min'] = w.LabelInput(
            advancedselectioninfo, 'Speed: min',
            field_spec=fields['Distance'],
            label_args={'foreground': 'black'},
            input_args={'width': 10, 'max_var': max_speed_var,
                        'focus_update_var': min_speed_var, })
        self.search_inputs['speed_min'].grid(row=0, column=4,
                                             padx=8, pady=(20, 0),
                                             sticky=(tk.W + tk.E))
        self.search_inputs['speed_max'] = w.LabelInput(
            advancedselectioninfo, 'Speed: max',
            field_spec=fields['Distance'],
            label_args={'foreground': 'black'},
            input_args={'width': 10,
                        'min_var': min_speed_var,
                        'focus_update_var': max_speed_var, })
        self.search_inputs['speed_max'].grid(row=1, column=4,
                                             padx=8, sticky=(tk.W + tk.E))
        # search button
        self.search_button = w.LabelInput(
            advancedselectioninfo, 'Search',
            input_class=ttk.Button,
...
class RecordList(tk.Frame):
    ...
    def on_sort_records(self, event):
        ...
        https://stackoverflow.com/questions/22032152/\
    ...
    def populate(self, rows):
        ...
        # selects automatically the first row, to make
        # selections keyboard-friendly
        if len(rows) > 0:
...
class DeleteTableForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks,
                 updated_table_names, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.callbacks = callbacks

        # marathon program information
        tableinfo = tk.LabelFrame(self, text='Marathon program information',
                                  padx=5, pady=5)
        ...
        self.deletebutton = w.LabelInput(
            tableinfo, 'Delete plan',
            input_class=ttk.Button,
...
class BarChartView(tk.Frame):
    def __init__(self, parent, fields, selection=1, *args, **kwargs):
        ...
        periods, distances, counts, average_speed = \
            self.fields(period=selection)
        distance_chart.draw_bar_chart(periods, distances,
                                      selection, 'dodgerblue')
        speed_chart.draw_bar_chart(periods, average_speed,
                                   selection, 'limegreen')
        count_chart.draw_bar_chart(periods, counts, selection,
                                   'gold', integer=True)
...
class StackedBarChartView(tk.Frame):
    def __init__(self, parent, table_name,
        ...
        distance_chart = w.BarChartWidget(self, "Week number",
                                          "Weekly distances (km)",
                                          "Weekly progression for " +
                                          title_name +
                                          " marathon training program",
                                          figsize=(15, 11))
        distance_chart.grid(row=0, column=0, sticky=(tk.W + tk.E))
        distance_chart.draw_stacked_bar_chart(self.days_of_week,
                                              self.weekly_distances)
        plotinfo.grid(row=0, column=0, sticky=(tk.W + tk.E))
