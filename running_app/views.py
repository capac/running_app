...
class DataInteractionForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks, api_data, *args, **kwargs):
        ...
        icon_url = 'http://openweathermap.org/img/wn/' + self.api_data['weather_0_icon'] + '@2x.png'
        raw_data = urllib.request.urlopen(icon_url).read()
        im = Image.open(BytesIO(raw_data)).resize((26, 26))
        # due to how python deals with garbage collection the picture just gets erased,
        # therefore a reference to the image in the global scope is required
        self.weather_icon_image = ImageTk.PhotoImage(im)
        ...
        self.weather_icon = ttk.Label(weatherpanel, image=self.weather_icon_image)
        self.weather_icon.grid(row=0, column=1, padx=2, sticky=tk.W)
...
class SearchForm(tk.Frame):
    ...
    def __init__(self, parent, fields, callbacks, valid_dates, *args, **kwargs):
        ...
        self.search_inputs['duration_min'].grid(row=0, column=1, padx=8, pady=(20, 0),
                                                sticky=(tk.W + tk.E))
        self.search_inputs['duration_max'] = w.LabelInput(advancedselectioninfo, 'Duration: max',
                                                          field_spec=fields['Search duration'],
                                                          input_args={'width': 10},)
        ...
        self.search_inputs['distance_min'] = w.LabelInput(advancedselectioninfo, 'Distance: min',
                                                          field_spec=fields['Distance'],
        ...
        self.search_inputs['distance_min'].grid(row=0, column=2, padx=8, pady=(20, 0),
                                                sticky=(tk.W + tk.E))
        self.search_inputs['distance_max'] = w.LabelInput(advancedselectioninfo, 'Distance: max',
                                                          field_spec=fields['Distance'],
                                                          input_args={'width': 10, 'min_var': min_distance_var,
                                                                      'focus_update_var': max_distance_var, })
        self.search_inputs['distance_max'].grid(row=1, column=2, padx=8, sticky=(tk.W + tk.E))
